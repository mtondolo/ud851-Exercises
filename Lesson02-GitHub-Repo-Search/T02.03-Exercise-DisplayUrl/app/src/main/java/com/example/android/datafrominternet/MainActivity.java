...
//        URL githubSearchUri = NetworkUtils.buildUrl ( githubQuery );
//        mUrlDisplayTextView.setText(githubSearchUri.toString());
//    }

    // COMPLETED (2) Create a method called makeGithubSearchQuery
    private void makeGithubSearchQuery() {
        // COMPLETED (3) Within this method, build the URL with the text from the EditText and set the built URL to the TextView
        String locationQuery = mSearchBoxEditText.getText ().toString ();
        URL githubSearchUri = NetworkUtils.buildUrl ( locationQuery );
        mUrlDisplayTextView.setText(githubSearchUri.toString());
    }
